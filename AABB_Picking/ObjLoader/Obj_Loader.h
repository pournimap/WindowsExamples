	...
	std::vector<float>& tangents, std::vector<float>& bitangents, std::vector<Mesh_Data> &mesh_data,char mtllib[256])
{
	...
	while (fgets(line_obj, BUFFER_SIZE, g_fp_meshfile) != NULL)
	{
		...
		first_token = strtok_s(line_obj, sep_space, &context);
		...
		if (strcmp(first_token, "v") == S_EQUAL)
		{
			...
			{
				vec_point_coord[i] = atof(strtok_s(NULL, sep_space, &context));	//S1, S2, S3
																	//vertices_arr[gvertices_count] = vec_point_coord[i];
																	//vertices_arr[gvertices_count] = atof(strtok_s(NULL, sep_space, &context));
																	//gvertices_count++;
			}
		...
		else if (strcmp(first_token, "vt") == S_EQUAL)
		{
			...
			for (int i = 0; i != NR_TEXTURE_COORDS; i++)
				vec_texture_coord[i] = atof(strtok_s(NULL, sep_space, &context));	//S1, S2, S3
		...
		else if (strcmp(first_token, "vn") == S_EQUAL)
		{
			...
			{
				vec_normal_coord[i] = atof(strtok_s(NULL, sep_space, &context));	//S1, S2, S3
			}
		...
		else if (strcmp(first_token, "usemtl") == S_EQUAL)
		{
			...
			token = strtok_s(NULL, sep_space, &context);
			strcpy_s(mesh_data_temp.usemtl, token);
